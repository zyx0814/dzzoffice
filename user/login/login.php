...
if ($_G['setting']['loginset']['template'] !== '4') {
    exit(json_encode(['error'=>'管理员已关闭此登录方式']));
}
if(isset($_GET['uid'])){
	$uid=intval($_GET['uid']);
	...
	if($user['adminid']==1){
        ...
        exit(json_encode(['error'=>'管理员禁止登陆']));
	}
    if ($user['status'] > 0) {
        writelog('loginlog', '用户ID：'.$_GET['uid'].' 使用登录模板4登录失败,此用户已禁用');
        exit(json_encode(['error'=>'此用户已禁用，请联系管理员']));
    }
    ...
    if($orgid == 'other') {
        ...
        if ($Users) {
            ...
            exit(json_encode(['error'=>'该用户不在允许登录的用户范围内']));
        }
    } elseif($orgid = intval($orgid)) {
        $Users = DB::result_first("SELECT COUNT(*) FROM %t WHERE orgid=%d AND uid=%d", ['organization_user', $orgid, $uid]);
        if(!$Users) {
            writelog('loginlog', '用户ID：'.$uid.' 使用登录模板4登录失败，不在允许的机构范围内');
            exit(json_encode(['error'=>'该用户不在允许登录的用户范围内']));
        }
    ...
    showTips(['success' => ['message' => lang('login_succeed_no_redirect'), 'url_forward' => $href]], 'json');
	exit();
}
showmessage('Access Denied',$_GET['referer']?:$_G['siteurl']);