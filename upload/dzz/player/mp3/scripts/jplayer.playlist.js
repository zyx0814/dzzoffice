...
(function($, undefined) {
	...
	jPlayerPlaylist.prototype = {
			...
			if(instant && !$.isFunction(instant)) {
				...
				$(this.cssSelector.playlist + " ul").slideUp(displayTime, function() {
					...
					if(self.playlist.length) {
						$(this).slideDown(self.options.playlistOptions.displayTime,function(){self._setWinSize();});
					} else {
						$(this).show();
						self._setWinSize();
					}
		...
		_savePlaylist: function(){
			jQuery.post(saveurl,{data:serialize(this.playlist)});
		},
		_setWinSize: function(width,height){
			try{
			win.ResizeTo(width?width:jQuery('#jp_container_1').outerWidth(),height?height:jQuery('#jp_container_1').outerHeight());
			}catch(e){}
		},
		...
		add: function(media, playNow) {
			var self=this;
			$(this.cssSelector.playlist + " ul").append(this._createListItem(media)).find("li:last-child").hide().slideDown(this.options.playlistOptions.addTime,function(){self._setWinSize();});
			this._updateControls();
				...
				if(this.removing) {
					...
					if(0 <= index && index < this.playlist.length) {
						...
						$(this.cssSelector.playlist + " li:nth-child(" + (index + 1) + ")").slideUp(this.options.playlistOptions.removeTime, function() {
							...
							//alert($('#jp_container_1').outerWidth()+'==='+$('#jp_container_1').outerHeight());
							self._setWinSize();
							if(self.shuffled) {
		...
		select: function(index) {
			...
			if(0 <= index && index < this.playlist.length) {
				...
				$('#jp-title-value').html(this.playlist[this.current].title);
				//api.setTitle(this.playlist[this.current].title);
			} else {
				this.current = 0;
			}
		},
		...
		shuffle: function(shuffled, playNow) {
			...
			if(shuffled || shuffled !== this.shuffled) {
				...
				$(this.cssSelector.playlist + " ul").slideUp(this.options.playlistOptions.shuffleTime, function() {
					...
					$(this).slideDown(self.options.playlistOptions.shuffleTime,function(){self._setWinSize();});
