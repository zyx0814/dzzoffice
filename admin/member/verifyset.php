...
if($do=='edit') {
	...
	if(!submitcheck('verifysubmit')) {
		...
		if($verifyarr['icon']) {
			...
			$verificonhtml = '<label class="radio-inline"><input type="checkbox" class="checkbox" name="deleteicon['.$vid.']" value="yes" />删 </label>&nbsp;<img src="'.$prefix.$verifyarr['icon'].'?t='.TIMESTAMP.'"  />';
		}
		$unverifyiconhtml = '';
		if($verifyarr['unverifyicon']) {
			...
			$unverifyiconhtml = '<label class="radio-inline"><input type="checkbox" class="checkbox" name="delunverifyicon['.$vid.']" value="yes" />删 </label>&nbsp;<img src="'.$prefix.$verifyarr['unverifyicon'].'?t='.TIMESTAMP.'" />';
		}
		
		$fieldarr=C::t('user_profile_setting')->fetch_all_by_available(1);
		unset($fieldarr['birthyear']);unset($fieldarr['birthmonth']);unset($fieldarr['zodiac']);unset($fieldarr['constellation']);		
		$groupselect = array();
		...
		if($verifynew['available'] == 1 && !trim($verifynew['title'])) {
			showmessage('认证名称不能为空', dreferer());
		}
		if($icon= getverifyicon('iconnew','common/verify/'.$vid.'/verify_icon.jpg')) $verifynew['icon']=$icon;
		else $verifynew['icon']=$_G['setting']['verify'][$vid]['icon'];
		if($uicon= getverifyicon('unverifyiconnew','common/verify/'.$vid.'/unverify_icon.jpg'))  $verifynew['unverifyicon']=$uicon;
		else $verifynew['unverifyicon']=$_G['setting']['verify'][$vid]['unverifyicon'];
		if($_GET['deleteicon']) {
		...
		showmessage('认证设置保存成功', ADMINSCRIPT.'?mod=member&op=verifyset', array(),array('alert'=>'right'));
	}
	...
	if(!submitcheck('verifysubmit')) {
		...
		showmessage('认证设置成功', dreferer(), array(),array('alert'=>'right'));
	}
...
function getverifyicon($iconkey = 'iconnew',$target) {
	...
	if($_FILES[$iconkey]) {
		...
		if($icon_url['host']) $iconnew =imagetolocal($_GET[''.$iconkey],'common',$target);
		else $iconnew=$_GET[''.$iconkey];
	}else{
		$iconnew='';
	}
	return $iconnew;
