   ...
   $groups = array();
    ...
    if ($newperpage) {
        ...
        foreach (C::t('resources')->fetch_all_by_pfid($fid, $conditions, $newperpage, $orderby, $order, $datastart) as $val) {
            if ($val['type'] == 'folder') {
				if(empty($val['contaions'])){
						$val['contaions']=C::t('resources')->get_contains_by_fid($val['oid']);
					}
    ...
    //$folderpatharr = array_combine($pathkeyarr,$folderpath);
    ...
    require template('mobile/file_content');
}
