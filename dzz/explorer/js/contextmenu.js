...
_contextmenu.right_ico = function (e, rid) {
    ...
    if (_filemanage.selectall.icos.length > 1 && jQuery.inArray(rid, _filemanage.selectall.icos) > -1) {
        ...
        if (collects == _filemanage.selectall.icos.length) {//区别是已收藏时，菜单显示取消收藏
            el.find('.collect .menu-text').html(__lang.cancel_collection);
        }
    } else {
        if (obj.collect) el.find('.collect .menu-text').html(__lang.cancel_collection);
        el.find('.downpackage').remove();
    }
    ...
    if (subdata === true) {
        ...
        for (var i = 0; i < subdata.length; i++) {
            html += '<div class="menu-item" onClick="_filemanage.Open(\'' + rid + '\',\'' + subdata[i].extid + '\');jQuery(\'#right_contextmenu\').hide();jQuery(\'#shadow\').hide();return false;">';
            if (subdata[i].icon) {
        ...
        /*//增加选择默认
         html+='<div class="menu-item" onClick="_ico.setOpenDefault(\''+icoid+'\');jQuery(\'#right_contextmenu\').hide();jQuery(\'#shadow\').hide();return false;">';
         html+='<span class="menu-icon icon-config"></span>';
         html+='<span class="menu-text">'+__lang.set_default_open+'</span>';
         html+='</div>';*/
