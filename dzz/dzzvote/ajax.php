...
if ($_GET['do'] == 'imageUpload') {
    ...
    $options = [
        ...
        'thumbnail' => ['max-width' => 240, 'max-height' => 160]
    ];
    ...
    $votesum = DB::result_first("select COUNT(DISTINCT uid) from %t where itemid IN(%n)", ['vote_item_count', $itemids]);
    if (DB::result_first("select COUNT(*) from %t where itemid IN(%n) and uid=%d", ['vote_item_count', $itemids, $_G['uid']]))
        $status = 1;
    if (!$_G['uid'])
        $status = 3;
    //游客无权投票；
    ...
    if ($itemids && $vote['showuser']) {//获取投票人
        if ($count = DB::result_first("select COUNT(DISTINCT uid) from %t where itemid IN(%n)", ['vote_item_count', $itemids])) {
            foreach (DB::fetch_all("select DISTINCT u.uid,u.username,u.avatarstatus,u.headerColor from %t c LEFT JOIN %t u ON c.uid=u.uid where itemid IN(%n) order by c.dateline DESC limit $start,$perpage", ['vote_item_count', 'user', $itemids]) as $value) {
    ...
    if (submitcheck('votesubmit')) {
        ...
        showmessage('do_success', DZZSCRIPT, [], ['showmsg' => true]);
    }
}
