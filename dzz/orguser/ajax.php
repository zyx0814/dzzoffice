...
if ($do == 'upload') {//上传图片文件
    ...
    $options = ['accept_file_types' => '/\.(gif|jpe?g|png)$/i',
        ...
        'thumbnail' => ['max-width' => 40, 'max-height' => 40]];
    ...
    if ($_GET['id'] == '#') {
        ...
        foreach (C::t('organization')->fetch_all_by_forgid($id, 0, 0) as $value) {
            ...
            $arr = ['id' => $value['orgid'], 'text' => $value['orgname'], 'icon' => $icon, 'state' => ['disabled' => $orgdisable], "type" => $orgtype, 'children' => true];
            if (intval($value['aid']) == 0) {
                $arr['text'] = avatar_group($value['orgid'], [$value['orgid'] => ['aid' => $value['aid'], 'orgname' => $value['orgname']]]) . $value['orgname'];
                $arr['icon'] = false;
            } else {
        ...
        if ($_G['adminid'] == 1) $data[] = ['id' => 'other', 'text' => lang('no_institution_users'), 'state' => ['disabled' => $disable], "type" => 'group'];
    } elseif ($id) {//获取用户列表
    ...
    $setarr = ['forgid' => intval($_GET['forgid']), 'orgname' => lang('new_department'), 'fid' => 0, 'disp' => intval($_GET['disp']), 'indesk' => 0, 'dateline' => TIMESTAMP, 'available' => 1, 'syatemon' => 1, 'manageon' => 1, 'maxspacesize' => getglobal('orgmemorySpace', 'setting')];
    ...
    $orgid = intval($_GET['orgid']);
    if (!$_GET['text']) {
        exit(json_encode(['msg' => lang('name_cannot_empty')]));
    }
    if (!$ismoderator = C::t('organization_admin')->ismoderator_by_uid_orgid($orgid, $_G['uid'])) {
        exit(json_encode(['error' => lang('no_privilege')]));
    }
    if (C::t('organization')->update_by_orgid($orgid, ['orgname' => $_GET['text']])) {
        exit(json_encode(['success' => true]));
    } else {
        exit(json_encode(['msg' => lang('rechristen_error')]));
    }
} elseif ($do == 'delete') {
    $forgid = intval($_GET['forgid']);
    if ($_GET['type'] == 'user') {//删除用户
        if ($_G['adminid'] != 1) exit(json_encode(['error' => '删除用户只限系统管理员操作']));
    ...
    if ($forgid) {
        ...
        if (C::t('organization_user')->delete_by_uid_orgid($uids, $forgid)) {
            ...
            exit(json_encode(['msg' => '移除失败，用户不存在或您没有权限移除此用户。<br>如果您移除的用户是机构部门管理员，需要上一级机构部门管理员才能移除此用户']));
        }
    } else {
        exit(json_encode(['error' => '请选择部门']));
    }
} elseif ($do == 'insert') {
    $uids = (array)$_GET['uids'];
    ...
    if ($_GET['type'] == 'user') {//移动用户
        ...
        if($_G['adminid'] != 1) {
            if (!C::t('organization_admin')->ismoderator_by_uid_orgid($orgid, $_G['uid'])) {
                exit(json_encode(['error' => '您没有目标部门的管理权限']));
            }
            if (!C::t('organization_admin')->ismoderator_by_uid_orgid($forgid, $_G['uid'])) {
                exit(json_encode(['error' => '您没有原部门的管理权限']));
            }
        }
        ...
        foreach ($uids as $uid) {
            if (C::t('organization_user')->move_to_by_uid_orgid($uid, $forgid, $orgid, $copy)) {
                ...
                exit(json_encode(['error' => lang('movement_error')]));
            }
        }
        exit(json_encode(['success' => true]));
    } else {
    ...
    if (C::t('organization_job')->update($jobid, ['name' => $name])) {
        exit(json_encode(['jobid' => $jobid, 'name' => $name]));
    } else {
        exit(json_encode(['error' => lang('edit_error')]));
    }
} elseif ($do == 'jobdel') {
    $jobid = intval($_GET['jobid']);
    $orgid = intval($_GET['orgid']);
    ...
    if (C::t('organization_job')->delete($jobid)) {
        exit(json_encode(['jobid' => $jobid]));
    } else {
        exit(json_encode(['error' => lang('delete_unsuccess')]));
    }
} elseif ($do == 'jobadd') {
    $orgid = intval($_GET['orgid']);
    if($_G['adminid'] != 1) {
    ...
    $name = str_replace('...', '', getstr($_GET['name'], 30));
    $setarr = ['orgid' => $orgid, 'name' => $name, 'dateline' => TIMESTAMP, 'opuid' => $_G['uid']];
    $setarr['jobid'] = C::t('organization_job')->insert_job_by_name($orgid, $name, $_G['uid']);
    if ($setarr['jobid']) {
        exit(json_encode($setarr));
    } else {
        exit(json_encode(['error' => lang('add_unsuccess')]));
    }
    ...
    if (C::t('organization')->setFolderAvailableByOrgid($orgid, intval($_GET['available']))) {
        ...
        exit(json_encode(['error' => lang('unable_set') . '，如果上级没有开启目录共享，下级无法开启']));
    }
    ...
    if (C::t('organization')->setIndeskByOrgid($orgid, intval($_GET['indesk']))) {
        ...
        exit(json_encode(['error' => lang('no_open_Shared_directory')]));
    }
    ...
    if(!$orgname) {
        exit(json_encode(['error' => lang('name_cannot_empty')]));
    }

    if (!C::t('organization_admin')->ismoderator_by_uid_orgid($orgid, $_G['uid'])) {
        exit(json_encode(['error' => lang('no_privilege')]));
    }
    if (C::t('organization')->update_by_orgid($orgid, ['orgname' => $orgname])) {
        exit(json_encode(['success' => true]));
    } else {
        exit(json_encode(['error' => lang('rechristen_error')]));
    }
    ...
    if (C::t('organization')->update_by_orgid($orgid, ['aid' => $img])) {
        exit(json_encode(['success' => true]));
    } else {
        exit(json_encode(['error' => lang('rechristen_error')]));
    }
    ...
    if (C::t('organization')->update_by_orgid($orgid, ['desc' => $desc])) {
        exit(json_encode(['success' => true]));
    } else {
        exit(json_encode(['error' => lang('rechristen_error')]));
    }
    ...
    if (C::t('organization')->setgroupByOrgid($orgid, intval($_GET['available']))) {
        ...
        exit(json_encode(['error' => lang('unable_set')]));
    }
} elseif ($do == 'orginfo') {
    $orgid = intval($_GET['orgid']);
    if(!$orgid) {
    ...
    if (!$org = C::t('organization')->fetch($orgid)) {
        exit(json_encode(['error' => '该机构或群组不存在或被删除']));
    }
    //暂时只允许系统管理员进行空间相关设置
    if ($_G['adminid'] != 1) {
        exit(json_encode(['error' => '没有权限']));
    }
    if ($setspacesize != 0) {
        //获取允许设置的空间值
        $allowallotspace = C::t('organization')->get_allowallotspacesize_by_orgid($orgid);
        if ($allowallotspace < 0) {
            exit(json_encode(['error' => '可分配空间不足']));
        }
        ...
        if ($setspacesize > 0 && $setspacesize * 1024 * 1024 < $currentallotspace) {
            exit(json_encode(['error' => '设置空间值不足,小于已分配空间值！', 'val' => $org['maxspacesize']]));
        }
        //上级包含空间限制时，无限制不处理，直接更改设置值
        if ($allowallotspace > 0 && ($setspacesize * 1024 * 1024 > $allowallotspace)) {
            exit(json_encode(['error' => '总空间不足！', 'val' => $org['maxspacesize']]));
        }
    }
    //设置新的空间值
    if (C::t('organization')->update($orgid, ['maxspacesize' => $setspacesize])) {
        exit(json_encode(['success' => true]));
    } else {
        exit(json_encode(['error' => '设置不成功或未更改', 'val' => $org['maxspacesize']]));
    }
} elseif ($do == 'guide') {
    ...
    $uid = intval($_GET['uid']);
    if (!$uid) {
        exit(json_encode(['error' => '用户不存在']));
    }
    if($uid == $_G['uid']) {
        exit(json_encode(['error' => '不能操作自己']));
    }
    $uperm = false;
    if ($_G['adminid'] != 1) {
        if ($orgids_uid = C::t('organization_user')->fetch_orgids_by_uid($uid)) {
            ...
            if (!$uperm) exit(json_encode(['error' => lang('no_privilege')]));
        } else {
            exit(json_encode(['error' => lang('no_privilege')]));
        }
    }
