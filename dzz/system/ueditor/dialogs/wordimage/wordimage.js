		...
		for (var i = 0,img; img = images[i++];) {
			...
			for (var j = 0,url; url = imageUrls[j++];) {
				if (src.indexOf(url.original.replace(" ","")) != -1) {
					...
					img.setAttribute("class",'dzz-image');
					img.setAttribute("path",'attach::'+url.attach.aid);
					img.setAttribute("apath",url.attach.apath);
					img.setAttribute("ext",url.attach.filetype);
					img.setAttribute("dsize",url.attach.filesize);
