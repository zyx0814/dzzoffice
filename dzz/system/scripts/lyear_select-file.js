...
_selectfile.Open = function (rid, extid, title) {
    ...
    if (extid) {
        ...
        parent.layer.alert('文件没有可以打开的应用', {
...
_selectfile.prototype.createIcosContainer = function () {
    ...
    jQuery('#' + containerid).off('click', '.detail_header:not(.detail_header_select)').on('click', '.detail_header:not(.detail_header_select)', function () {
		_selectfile.Disp(this, self.id, parseInt(jQuery(this).attr('disp')));
	});
    ...
    jQuery(document).on('mousedown.file_text_' + id, function (e) {
        ...
        if (jQuery(obj).closest('#file_text_' + id).length < 1) {
            ...
            if (emptymatch.test(text)) {
                parent.layer.confirm(__lang.name_is_must, {skin:'lyear-skin-danger'}, function(){
    ...
    jQuery.ajax({
        ...
        success: function (json) {
            if (json.rid) {
                ...
                if (json.error) {
                    parent.layer.msg(json.error, {icon: 'error',skin: 'bg-danger',offset:'t'});
                }
            }
        },
        error: function () {
            ...
            parent.layer.msg(__lang.js_network_error, {icon: 'error',skin: 'bg-danger',offset:'t'});
        }
    });
};
...
//        showWindow('newFolder', _explorer.appUrl + '&do=ajax&operation=' + type + '&fid=' + fid);
		  $.post(_explorer.appUrl + '&do=ajax&operation=newFolder&template=1',{'fid':fid},function(data){
		  	if(data.msg === 'success'){
...
_selectfile.copy = function (rid) {
    ...
    jQuery.post(url, data, function (json) {
        if (json.msg === 'success') {
            ...
            parent.layer.msg(filenames + __lang.copy_success, {icon: 'success',skin: 'bg-primary',offset:'t'});
        } else {
...
_selectfile.cut = function (rid) {
    ...
    jQuery.post(url, data, function (json) {
        if (json.msg === 'success') {
            ...
            filenames = filenames.substr(0, filenames.length - 1);
            parent.layer.msg(filenames + __lang.cut_success, {icon: 'success',skin: 'bg-primary',offset:'t'});
        } else {
            parent.layer.msg(json.msg, {icon: 'error',skin: 'bg-danger',offset:'t'});
        }
...
_selectfile.delIco = function (rid, noconfirm) {
    ...
    if (!noconfirm) {
        ...
        if (_selectfile.selectall.icos.length > 0 && jQuery.inArray(rid, _selectfile.selectall.icos) > -1) {
            if (_explorer.sourcedata.icos[_selectfile.selectall.icos[0]].isdelete > 0 || (_explorer.sourcedata.icos[_selectfile.selectall.icos[0]].bz && _explorer.sourcedata.icos[_selectfile.selectall.icos[0]].bz)) {
                parent.layer.confirm((finallydelete) ? __lang.js_finallydelete_selectall : __lang.js_delete_selectall, {skin:'lyear-skin-danger'}, function(){
                    _selectfile.delIco(rid, 1);
                });
            } else {
                parent.layer.confirm((finallydelete) ? __lang.js_finallydelete_selectall_recycle : __lang.js_delete_selectall_recycle, {skin:'lyear-skin-danger'}, function(){
                    _selectfile.delIco(rid, 1);
                });
            }
            return;
            ...
            if (_explorer.sourcedata.icos[rid].isdelete > 0 || (_explorer.sourcedata.icos[rid].bz && _explorer.sourcedata.icos[rid].bz)) {
                parent.layer.confirm((finallydelete) ? __lang.js_finallydelete_folder.replace('{name}', _explorer.sourcedata.icos[rid].name) : __lang.js_delete_folder.replace('{name}', _explorer.sourcedata.icos[rid].name), {skin:'lyear-skin-danger'}, function(){
                    _selectfile.delIco(rid, 1);
                });
            } else {
                parent.layer.confirm((finallydelete) ? __lang.js_finallydelete_folder_recycle.replace('{name}', _explorer.sourcedata.icos[rid].name) : __lang.js_delete_folder_recycle.replace('{name}', _explorer.sourcedata.icos[rid].name), {skin:'lyear-skin-danger'}, function(){
                    _selectfile.delIco(rid, 1);
                });
            }
            return;
            ...
            if (_explorer.sourcedata.icos[rid].isdelete > 0 || (_explorer.sourcedata.icos[rid].bz && _explorer.sourcedata.icos[rid].bz)) {
                parent.layer.confirm((finallydelete) ? __lang.js_finallydelete_confirm.replace('{name}', _explorer.sourcedata.icos[rid].name) : __lang.js_delete_confirm.replace('{name}', _explorer.sourcedata.icos[rid].name), {skin:'lyear-skin-danger'}, function(){
                    _selectfile.delIco(rid, 1);
                });
            } else {
                parent.layer.confirm((finallydelete) ? __lang.js_finallydelete_confirm_recycle.replace('{name}', _explorer.sourcedata.icos[rid].name) : __lang.js_delete_confirm_recycle.replace('{name}', _explorer.sourcedata.icos[rid].name), {skin:'lyear-skin-danger'}, function(){
                    _selectfile.delIco(rid, 1);
                });
            }
            return;
    ...
    parent.layer.msg('<p>' + __lang.deleting_not_please_close + '</p>', {icon: 'success',skin: 'bg-primary',offset:'t'});
    jQuery.post(url, data, function (json) {
        var rids = [];
        for (var i in json.msg) {
            if (json.msg[i] === 'success') {
                parent.layer.msg(_explorer.sourcedata.icos[i].name + __lang.delete_success, {icon: 'success',skin: 'bg-primary',offset:'t'});
                rids.push(i);
                total--;
            } else {
                parent.layer.msg(json.msg[i], {icon: 'error',skin: 'bg-danger',offset:'t'});
            }
        }
