...
class io_dzz extends io_api {
    ...
    public function rename($rid, $text) {
        ...
        if ($icoarr['name'] != $text && ($ricoid = $this->getRepeatIDByName($text, $icoarr['pfid'], $icoarr['type'] == 'folder'))) {//如果目录下有同名文件
            return ['error' => lang('filename_already_exists')];
        }
    ...
    public function createThumb($path, $size, $width = 0, $height = 0, $thumbtype = 1) {
        ...
        $imgcachePath = 'imgcache/';
        $cachepath = str_replace('//', '/', str_replace(':', '/', $data['attachment'] ?: $data['path']));
    ...
    public function setFileContent($rid, $fileContent, $force = false, $nocover = true) {
        ...
        if ($icoarr['isdelete']) {
            return ['error' => lang('file_been_deleted')];
        }
        ...
        if (!$folder = C::t('folder')->fetch($icoarr['pfid'])) {
            return ['error' => lang('parent_directory_not_exist')];
        }
        if (!$force) {
        ...
        if (!$attach = getTxtAttachByMd5($fileContent, $icoarr['name'], $icoarr['ext'])) {
            return ['error' => lang('file_save_failure')];
        }
        $covertype = 0;
        if ($nocover) {//判断是否是覆盖
            $setting = $_G['setting'];
            //当前文件版本数量
            $versionnum = DB::result_first("select count(*) from %t where rid = %s", ['resources_version', $icoarr['rid']]);
            //版本开启
        ...
        if ($covertype) {
            if ($covertype == 2) {
                $vinfo = DB::fetch_first("select min(dateline),vid from %t where rid = %s ", ['resources_version', $icoarr['rid']]);
                C::t('resources_version')->delete_by_vid($vinfo['vid'], $icoarr['rid']);
            }
            $setarr = [
                'uid' => $_G['uid'] ?: $folder['uid'],
            ...
            if ($csize) {
                if (!SpaceSize($csize, $folder['gid'], 0, $icoarr['uid'])) {
                    return ['error' => lang('inadequate_capacity_space')];
                }
                SpaceSize($csize, $folder['gid'], 1, $icoarr['uid']);
            }
            //更新附件数量
            if ($icoarr['aid'] != $attach['aid']) {
                C::t('resources')->update_by_rid($rid, ['size' => $attach['filesize']]);
                C::t('resources_statis')->add_statis_by_rid($rid, ['editdateline' => TIMESTAMP]);
                C::t('resources_attr')->update_by_skey($icoarr['rid'], $icoarr['vid'], ['aid' => $attach['aid']]);
                C::t('attachment')->update($attach['aid'], ['copys' => $attach['copys'] + 1]);
    ...
    public function getRepeatIDByName($filename, $fid, $isfolder = false) {
        ...
        if ($rid = DB::result_first("select rid from %t where $sql ", ['resources', $fid, $filename])) {
            return $rid;
        } else return false;
    }
    ...
    public function zipdownload($paths, $filename = '', $checkperm = true) {
        ...
        $paths = (array)$paths;
        set_time_limit(0);
        $eventdata = ['username' => $_G['username'], 'dateline' => TIMESTAMP];
        ...
        if (count($paths) > 1) {
            $filenames = '';
            foreach (DB::fetch_all("select name from %t where rid in(%n)", ['resources', $paths]) as $v) {
                $filenames .= $v['name'] . ',';
            }
            ...
            foreach ($paths as $path) {
                ...
                switch ($meta['type']) {
                    case 'folder':
                        ...
                        foreach ($rids as $rid) {
                            $this->getFolderInfo([$rid], $lposition, $zip,$checkperm);
                        }
                        break;
                    case 'discuss':
                    case 'shortcut':
    ...
    public function download($paths, $filename = '', $checkperm = true) {
        ...
        if (strpos($path, 'attach::') === 0) {
            ...
            $attachment['name'] = $filename ?: $attachment['filename'];
            $path = getDzzPath($attachment);
            $attachurl = IO::getStream($path);
        } elseif (strpos($path, 'dzz::') === 0) {
            $attachment = ['attachment' => preg_replace("/^dzz::/i", '', $path), 'name' => $filename ?: substr($path, strrpos($path, '/') + 1)];
            $attachurl = $_G['setting']['attachdir'] . $attachment['attachment'];
        } elseif (strpos($path, 'TMP::') === 0) {
            $tmp = str_replace('\\', '/', sys_get_temp_dir());
            $attachurl = str_replace('TMP::', $tmp . '/', $path);
            $pathinfo = pathinfo($attachurl);
            $attachment = ['attachment' => $attachurl, 'name' => $filename ?: $pathinfo['basename']];
        } elseif (preg_match('/\w{32}/i', $path)) {
            $icoid = trim($path);
            $icoarr = C::t('resources')->fetch_by_rid($path);
    ...
    public function Delete($path, $finaldelete = false, $force = false) {
        ...
        if (strpos($path, 'dzz::') === 0) {
            ...
            if ($rid = DB::result_first("select rid from %t where pfid = %d and name = %s and isdelete < 1", ['resources', $pfid, $filename])) {
                $icoarr = C::t('resources')->fetch_by_rid($rid);
            } else {
                return ['error' => lang('file_longer_exists')];
            }
            ...
            if ($force || perm_check::checkperm('delete', $icoarr)) {
                ...
                return ['rid' => $icoarr['rid'], 'error' => lang('file_delete_no_privilege')];
            }

            return ['rid' => $icoarr['rid'], 'name' => $icoarr['name']];
        } elseif (preg_match('/\w{32}/i', $path)) {//rid删除
            try {
                if (!$icoarr = C::t('resources')->fetch_by_rid($path)) {
                    return ['rid' => $path, 'error' => lang('file_longer_exists')];
                }
    ...
    public function getFolderName($name, $pfid) {
        ...
        if (DB::result_first("select COUNT(*) from %t where fname=%s and  pfid=%d and isdelete<1", ['folder', $name, $pfid])) {
    ...
    //根据文件名创建顶级目录
    public function createTopFolderByFname($fname, $perm = 0, $params = [], $ondup = 'newcopy') {
        global $_G;
        if (!$_G['uid']) {
            return ['error' => lang('no_privilege')];
        }
        $folderparams = ['innav', 'fsperm', 'disp', 'iconview', 'display', 'flag', 'default', 'perm'];
        $data = [];
    ...
    //创建目录
    public function CreateFolder($pfid, $fname, $perm = 0, $params = [], $ondup = 'newcopy', $force = false) {
        if (!$fname) {
            return ['error' => lang('directory_name_can_not_empty')];
        }
        global $_G;
        $folderparams = ['innav', 'fsperm', 'disp', 'iconview', 'display', 'flag', 'default', 'perm'];
        if ($pfid == 0) {
        ...
        if (!$force && !perm_check::checkperm_Container($pfid, 'folder', '' , $folder['uid'])) {
            return ['error' => lang('folder_newfolder_no_privilege')];
        }
    ...
    public function save($target, $filename = '') {
        ...
        if ($md5 && $attach = DB::fetch_first("select * from %t where md5=%s and filesize=%d", ['attachment', $md5, $filesize])) {
            ...
            if ($attach['aid'] = C::t('attachment')->insert($attach, 1)) {
                ...
                if ($_G['setting']['thumb_active'] && $remoteid < 2 && in_array($attach['filetype'], ['jpg', 'jpeg', 'png'])) {//主动模式生成缩略图
                    try {
        ...
        if (in_array(strtolower($attach['filetype']), $imgexts)) {
            ...
            if ($icoarr['rid'] = C::t('resources')->insert_data($icoarr)) {//插入主表
                ...
                if (C::t('resources_attr')->insert_attr($icoarr['rid'], $icoarr['vid'], $sourceattrdata)) {//插入属性表
                    C::t('attachment')->update($attach['aid'], ['copys' => $attach['copys'] + 1]);//增加图片使用数
            ...
            if ($icoarr['rid'] = C::t('resources')->insert_data($icoarr)) {
                C::t('attachment')->update($attach['aid'], ['copys' => $attach['copys'] + 1]);//增加文档使用数
                $sourcedata = [
                    'title' => $attach['filename'],
                    'desc' => '',
                    'aid' => $attach['aid'],
    ...
    public function CreateFolderByPath($path, $pfid, $params = [], $force = false) {
        $data = ['pfid' => $pfid];
        if (!$path) {
            $data['pfid'] = $pfid;
        } else {
            ...
            foreach ($patharr as $fname) {
                if (!$fname) continue;
                //判断是否含有此目录
                if ($fid = DB::result_first("select fid from %t where pfid=%d and isdelete<1 and fname=%s", ['folder', $pfid, $fname])) {
                    $pfid = $data['pfid'] = $fid;
                } elseif ($re = $this->CreateFolder($data['pfid'], $fname, 0, $params, 'overwrite',$force)) {
    ...
    public function uploadStream($file, $filename, $pfid, $relativePath = '', $content_range = [], $force = false, $ondup = 'newcopy') {
        $data = [];
        $params = [];
        ...
        if (substr($filename, -7) == '.folder') {
            $data = $this->CreateFolderByPath($relativePath ?: substr($filename, 0, -7), $pfid, $params,$force);
            if (isset($data['error'])) {
                return ['error' => $data['error']];
            }
        ...
        if ($arr['ispart']) {
            ...
            $re = $this->upload($fileContent, $data['pfid'], $filename, [], $ondup, $force);
            if (empty($re['error'])) {
    ...
    public function upload_by_content($fileContent, $path, $filename, $partinfo = [], $force = false) {
     ...
     * @return string
     */
    public function upload($fileContent, $fid, $filename, $partinfo = [], $ondup = 'newcopy', $force = false) {
        global $_G;
        ...
        if (($ondup == 'overwrite' || $ondup == 'ignore') && ($rid = $this->getRepeatIDByName($filename, $fid))) {//如果目录下有同名文件
            if ($ondup == 'overwrite') {//覆盖
                ...
                return ['error' => lang('parameters_error')];
            }
        } else {
        ...
        if ($partinfo['ispart']) {
            ...
            if (file_put_contents($_G['setting']['attachdir'] . $target,$fileContent,FILE_APPEND) === false) {
                return ['error' => lang('cache_file_error')];
            }
    ...
    public function overwriteUpload($fileContent, $rid, $filename, $partinfo = [], $force = false) {
        ...
        if (!$attach = $this->save($target, $icoarr['name'])) {
            return ['error' => lang('file_save_exist')];
        }
    ...
    public static function dzz_imagetoattach($link, $gid) {
        ...
        if ($md5 && $attach = C::t('attachment')->fetch_by_md5($md5)) {
            ...
            return $attach;
        } elseif ($target = imagetolocal($link, 'dzz')) {
            //判断空间大小
    ...
    public static function linktourl($link, $pfid,$name = '') {
        ...
        $clink['title'] = IO::getFileName($data['title'] ?: $parseurl['host'], $fid);
        ...
        if ($icoarr['rid'] = C::t('resources')->insert_data($icoarr)) {
            ...
            if (C::t('resources_attr')->insert_attr($icoarr['rid'], $icoarr['vid'], $sourcedata)) {
                C::t('collect')->update($clink['cid'], ['copys' => $clink['copys'] + 1]);
                ...
                C::t('resources')->delete_by_rid($icoarr['rid']);
                return ['error' => lang('linktourl_error')];
            }
        }
    ...
    public function RecovercreateFolderByPath($path, $pfid, $params = []) {
        $data = ['pfid' => $pfid];
        if (!$path) {
            $data['pfid'] = $pfid;
        } else {
            ...
            foreach ($patharr as $fname) {
                ...
                $path = $fpath . $fname . '/';
                $fid = DB::result_first("select fid from %t where path = %s", ['resources_path', $path]);
                /* echo $fid;
                 die;*/
                //判断是否含有此目录
                if ($finfo = DB::fetch_first("select f.fid,f.isdelete,r.rid from %t f
                  left join %t r on f.fid=r.oid where f.fid=%d  and f.fname=%s", ['folder', 'resources', $fid, $fname])
                ) {
                    if ($finfo['isdelete'] > 0) {
                        if ($finfo['rid']) {
                            ...
                            C::t('resources')->update_by_rid($finfo['rid'], ['isdelete' => 0, 'deldateline' => 0, 'pfid' => $pfid]);
                        }
                        C::t('folder')->update($finfo['fid'], ['isdelete' => 0, 'deldateline' => 0, 'pfid' => $pfid]);
                    }
                    $pfid = $data['pfid'] = $finfo['fid'];
                } elseif ($re = $this->CreateFolder($data['pfid'], $fname, 0, $params, 'overwrite')) {
                    if($re['error']) return $re;
                    $data['icoarr'][] = $re['icoarr'];
                    $data['folderarr'][] = $re['folderarr'];
                    $pfid = $data['pfid'] = $re['folderarr']['fid'];
    ...
    public function Recover($rid, $combine = true, $force = false) {
        ...
        if (!$force && !perm_check::checkperm('delete', $icoarr)) {
            ...
            if ($icoarr['type'] == 'folder') {
                ...
                $ricoid = $this->getRepeatIDByName($icoarr['name'], $icoarr['pfid'], true);
                if ($combine && $ricoid) {
                    $rinfo = C::t('resources')->fetch_info_by_rid($ricoid);
                    //目录下所有删除文件rid
                    $rids = [];
                    $fids = [];
                    foreach (DB::fetch_all("select rid,type,oid from %t where pfid = %d and isdelete > 0", ['resources', $icoarr['oid']]) as $v) {
                        if ($v['type'] == 'folder') {
                            $fids[] = $v['oid'];
                        } else {
                            $rids[] = $v['rid'];
                    ...
                    if (count($rids) > 0) C::t('resources')->update_by_rid($rids, ['pfid' => $rinfo['oid']]);
                    if (count($fids) > 0) C::t('folder')->update($fids, ['pfid' => $rinfo['oid']]);
                    //更改当前目录下所有下级文件路径
                    ...
                    //改变分享表数据
                    DB::update('shares', ['pfid' => $rinfo['oid']], ['pfid' => $icoarr['oid']]);
                    ...
                    if ($icoarr['isdelete'] > 0) {
                        ...
                        if (C::t('resources')->update_by_rid($rid, $recoverarr)) {
                            ...
                            $hash = C::t('resources_event')->get_showtpl_hash_by_gpfid($icoarr['pfid'], $icoarr['gid']);
                            //添加事件
                            $eventdata1 = ['username' => $_G['username'], 'position' => $path, 'filename' => $icoarr['name'], 'hash' => $hash];
                    ...
                    //目录下所有删除文件rid
                    $rids = [];
                    foreach (DB::fetch_all("select rid from %t where pfid = %d and isdelete > 0", ['resources', $icoarr['oid']]) as $v) {
                        $rids[] = $v['rid'];
                    }
                    foreach ($rids as $v) {
                        $this->Recover($v, $combine, $force);
                ...
                $totalsize = 0;
                if ($icoarr['vid'] > 0) {
                    $totalsize = DB::result_first("select sum(size) from %t where rid = %s", ['resources_version', $icoarr['rid']]);
                } else {
        ...
        if ($icoarr = C::t('resources')->fetch($rid)) {
            ...
            if ($icoarr['pfid'] != $tfolder['fid']) {
                ...
                if (!$force) {
                    ...
                    } elseif (!perm_check::checkperm('delete', $icoarr)) {
            ...
            if ($icoarr['type'] == 'folder') {
                ...
                if ($currentfid = DB::result_first("select oid from %t where pfid = %d and `name` = %s and `type` = %s and isdelete < 1",
                    ['resources', $tfolder['fid'], $icoarr['name'], 'folder'])
                ) {
                    //移动源文件夹数据到目标目录同名文件夹
                    ...
                    //删除文件夹表数据
                    DB::delete('folder', ['fid' => $icoarr['oid']]);
                    //删除事件表数据
                    C::t('resources_event')->delete_by_rid($icoarr['rid']);

                    //添加事件
                    $oldhash = C::t('resources_event')->get_showtpl_hash_by_gpfid($oldpfid, $oldgid);
                    $eventdata1 = ['username' => $_G['username'], 'olderposition' => $oldpathstr, 'newposition' => $targetstr, 'foldername' => $icoarr['name'], 'hash' => $oldhash];
                    ...
                    $eventdata2 = ['username' => $_G['username'], 'newposition' => $targetstr, 'foldername' => $icoarr['name'], 'hash' => $hash];
                    C::t('resources_event')->addevent_by_pfid($oldpfid, 'move_folder', 'movefolder', $eventdata2, $ogid, $rid, $icoarr['name']);
                } elseif ($folder = C::t('folder')->fetch($icoarr['oid'])) {//查询源文件夹数据
                    ...
                    $rids = [];
                    foreach (DB::fetch_all("select rid from %t where pfid in(%n) or oid in(%n)", ['resources', $fids, $fids]) as $v) {
                        $rids[] = $v['rid'];
                    }
                    //修改文件夹表数据和resources表数据
                    if (C::t('folder')->update($folder['fid'], $folder) &&
                        C::t('resources')->update_by_rid($rid, ['oid' => $folder['fid'], 'pfid' => $pfid, 'gid' => $gid, 'uid' => $_G['uid'], 'username' => $_G['username']])
                    ) {
                        //更改文件夹路径
                ...
                $icoarr['gid'] = $gid;
                $icoarr['uid'] = $_G['uid'];
                $icoarr['username'] = $_G['username'] ?: $_G['clientip'];
                $icoarr['pfid'] = $pfid;
                $icoarr['isdelete'] = 0;
                ...
                if (C::t('resources')->update_by_rid($icoarr['rid'], $icoarr)) {
                    ...
                    //添加移动文件动态
                    $oldhash = C::t('resources_event')->get_showtpl_hash_by_gpfid($oldpfid, $oldgid);
                    $eventdata1 = ['username' => $_G['username'], 'olderposition' => $oldpathstr, 'newposition' => $targetstr, 'filename' => $icoarr['name'], 'hash' => $oldhash];
                    C::t('resources_event')->addevent_by_pfid($pfid, 'moved_file', 'movedfile', $eventdata1, $gid, $rid, $icoarr['name']);

                    $hash = C::t('resources_event')->get_showtpl_hash_by_gpfid($icoarr['pfid'], $ogid);
                    $eventdata2 = ['username' => $_G['username'], 'newposition' => $targetstr, 'filename' => $icoarr['name'], 'hash' => $hash];
            ...
            C::t('resources')->update_by_rid($icoarr['rid'], ['isdelete' => 0, 'deldateline' => 0]);
            //addtoconfig($icoarr);
        }
    ...
    public function FileCopy($rid, $pfid, $first = true, $force = false) {
        ...
        if ($icoarr = C::t('resources')->fetch_by_rid($rid)) {
            ...
            if (!$force) {
                ...
                } elseif (!perm_check::checkperm('copy', $icoarr)) {
            ...
            if ($icoarr['type'] == 'folder') {
                ...
                if ($icoarr['pfid'] == $pfid) {//判断源文件位置和目标位置是否相同,如果相同则生成副本
                    ...
                    if ($ricoid = $this->getRepeatIDByName($icoarr['name'], $pfid, $icoarr['type'] == 'folder')) {//如果目录下有同名文件
                ...
                if ($folder = C::t('folder')->fetch($icoarr['oid'])) {
                    ...
                    if ($icoarr['pfid'] != $pfid && $currentinfo = DB::fetch_first("select oid,rid from %t where pfid = %d and `name` = %s and `type` = %s and isdelete < 1", ['resources', $tfolder['fid'], $icoarr['name'], 'folder'])) {
                        ...
                        $icoarr['rid'] = $data['rid'];
                    } elseif ($data = $this->CreateFolderByPath($icoarr['name'], $pfid)) {//如果目标目录中不存在同名文件夹或者存在同名文件夹而源文件位置和目标位置在同一目录，执行创建，根据文件夹名字和当前文件夹路径创建文件夹
                ...
                if ($icoarr['rid'] = C::t('resources')->insert_data($setarr)) {
                    ...
                    if (C::t('resources_attr')->insert_attr($icoarr['rid'], $setarr['vid'], $sourceattrdata)) {//插入属性表
                        if ($icoarr['aid']) {
                            ...
                            C::t('attachment')->update($icoarr['aid'], ['copys' => $attach['copys'] + 1]);//增加使用数
                        }
                        if ($icoarr['oid']) {
                            C::t('collect')->update($icoarr['oid'], ['copys' => $collect['copys'] + 1]);//增加使用数
                        }
            ...
            if ($icoarr['rid']) {
                ...
                return ['error' => lang('files_allowed_copy')];
            }
    ...
    public function multiUpload($opath, $path, $filename, $attach = [], $ondup = "newcopy") {
        ...
        */
        ...
        if (strpos($path, 'dzz::') === false && strpos($path, 'TMP::') === false) {
            $gid = DB::result_first("select gid from %t where fid=%d", ['folder', $path]);
            if (!SpaceSize($size, $gid)) {
                return ['error' => lang('inadequate_capacity_space')];
            }
        }
