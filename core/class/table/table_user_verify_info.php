...
class table_user_verify_info extends dzz_table {
    ...
    public function fetch_by_uid_verifytype($uid, $verifytype) {
        return DB::fetch_first('SELECT * FROM %t WHERE uid=%d AND verifytype=%d', [$this->_table, $uid, $verifytype]);
    }

    public function fetch_all_search($uid, $vid, $flag = null, $username = '', $starttime = 0, $endtime = 0, $order = 'dateline', $start = 0, $limit = 0, $sort = 'DESC', $orgids = []) {
    ...
    public function group_by_verifytype_count() {
        return DB::fetch_all('SELECT verifytype, COUNT(*) AS num FROM %t WHERE flag=0 GROUP BY verifytype', [$this->_table]);
    }
    ...
    public function delete_by_uid($uid, $verifytype = null) {
        if ($uid) {
            $addsql = '';
            if ($verifytype !== null) {
                $verifytype = dintval($verifytype, is_array($verifytype));
                $addsql = ' AND ' . DB::field('verifytype', $verifytype);
            }
            return DB::query('DELETE FROM %t WHERE ' . (is_array($uid) ? 'uid IN(%n)' : 'uid=%d') . $addsql, [$this->_table, $uid]);
        }
        return false;
    }

    public function count_by_search($uid, $vid, $flag = null, $username = '', $starttime = 0, $endtime = 0, $orgids = []) {
        $condition = $this->search_condition($uid, $vid, $flag, $username, $starttime, $endtime, $orgids);
