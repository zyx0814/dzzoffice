...
class table_organization_admin extends dzz_table {
    ...
    public function insert($uid, $orgid = false, $admintype = 1, $silent = false) {
        ...
        $id = parent::insert(["orgid" => $orgid, 'uid' => $uid, 'opuid' => getglobal('uid'), 'dateline' => TIMESTAMP, 'admintype' => $admintype], 1, 1);
        self::update_groupid_by_uid($uid);


        return DB::result_first('select id from %t where uid=%d and orgid=%d', [$this->_table, $uid, $orgid]);
    }

    public function update_perm($uid, $orgid, $admintype) {
        if (DB::result_first("select count(*) from %t where orgid = %d and uid = %d", [$this->_table, $orgid, $uid])) {
            if ($admintype == 0) {
                return DB::delete($this->_table, ['orgid' => $orgid, 'uid' => $uid]);
            } else {
                return DB::update($this->_table, ['admintype' => $admintype], ['orgid' => $orgid, 'uid' => $uid]);
            }
        } else {
    ...
    public function is_admin_by_orgid($orgid, $uid) {
        $currentpathkey = DB::result_first("select pathkey from %t where orgid = %d", ['organization', $orgid]);
        $orgids = explode('-', str_replace('_', '', $currentpathkey));
        if (DB::result_first("select count(*) from %t where orgid in (%n) and uid = %d", [$this->_table, $orgids, $uid]) > 0) {
            return true;
        }
        return false;
    }
    ...
    public function fetch_group_creater($orgid) {
        ...
        $uid = DB::result_first("select uid from %t where orgid = %d and admintype = %d", [$this->_table, $orgid, 2]);
        return DB::result_first("select username from %t where uid = %d ", ['user', $uid]);
    }
    ...
    public function update_groupid_by_uid($uid) {
        ...
        //判断当前用户是否仍为机构和部门管理员
        if (DB::result_first("select COUNT(*) from %t a left join %t o on o.orgid = a.orgid where a.uid=%d and o.type = 0 ", [$this->_table, 'organization', $uid])) {
            $groupid = 2;
        } else {
            $groupid = 9;
        }
        return C::t('user')->update($uid, ['groupid' => $groupid]);
    }
    ...
    public function delete_by_orgid($orgids) {
        $orgids = (array)$orgids;
        $uids = [];
        foreach (DB::fetch_all("select uid from %t where orgid IN (%n) ", [$this->_table, $orgids]) as $value) {
    ...
    public function fetch_moderators_by_orgid($orgids, $count = false) {
        ...
        return DB::fetch_all("select o.* ,u.username,u.email,u.uid,u.avatarstatus,u.headerColor from " . DB::table($this->_table) . " o LEFT JOIN " . DB::table('user') . " u ON o.uid=u.uid where o.orgid IN(" . dimplode($orgids) . ") order by o.dateline DESC");
    }
    ...
    public function ismoderator_by_uid_orgid($orgid, $uid, $up = 1) {
        ...
        return DB::result_first("select COUNT(*) from %t where orgid IN (%n) and uid=%d ", [$this->_table, $orgids, $uid]);
    }
    ...
    public function chk_memberperm($orgid, $uid = 0) {
        ...
        if ($admintype = DB::result_first("select admintype from %t where orgid = %d and uid = %d", [$this->_table, $orgid, $uid])) {
            $perm = $admintype;
        }
        return $perm;
    }
    ...
    public function fetch_adminer_by_orgid($orgid) {
        $admindata = '';
        foreach (DB::fetch_all("select u.username from %t a left join %t u on a.uid = u.uid where orgid = %d ", [$this->_table, 'user', $orgid]) as $v) {
            $admindata .= $v['username'] . ',';
        }
        return substr($admindata, 0, -1);
    }
}
