...
class table_resources_path extends dzz_table {
    ...
    public function fetch_pathby_pfid($pfid, $pathkey = false) {
        ...
        if ($data = DB::fetch_first("select {$fileds} from %t where fid = %d", [$this->_table, $pfid])) {
    ...
    public function update_by_fid($fid, $setarr) {
        if (DB::update($this->_table, $setarr, ['fid' => $fid])) {
    ...
    public function fetch_fid_bypath($path) {
        if ($data = DB::fetch_first("select p.fid from %t p left join %t f on f.fid = p.fid  where p.path = %s and f.isdelete < %d", [$this->_table, 'folder', $path, 1])) {
            return $data['fid'];
        }
        return false;
    }
    ...
    public function delete_by_path($path) {
        ...
        $fids = [];
        foreach (DB::fetch_all("select fid from %t where path regexp %s", [$this->_table, '^' . $path . '.*']) as $v) {
            $fids[] = $v['fid'];
        }
    ...
    public function fetch_folder_containfid_by_pfid($pfid) {
        ...
        $results = DB::fetch_all("select * from %t where pathkey regexp %s", [$this->_table, '^' . $pathkey . '.+']);
    ...
    public function fetch_by_path($path, $prefix = '', $uid = 0) {
        ...
        if ($prefix) {
            switch ($prefix) {
                case  'g':
                    $orgid = DB::result_first("select orgid from %t where orgname = %s and forgid = %d and `type` = %d", ['organization', $patharr[0], 0, 1]);
                    $path = 'dzz:gid_' . $orgid . ':' . $path;
                    $fid = DB::result_first("select fid from %t where path = %s ", [$this->_table, $path]);
                    break;
                case  'o':
                    $orgid = DB::result_first("select orgid from %t where orgname = %s and forgid = %d and `type` = %d", ['organization', $patharr[0], 0, 0]);
                    $path = 'dzz:gid_' . $orgid . ':' . $path;
                    $fid = DB::result_first("select fid from %t where path = %s ", [$this->_table, $path]);
                    break;
                case  'c':
                    $fid = 'c_' . DB::result_first("select id from %t where uid = %d and catname = %s", ['resources_cat', $uid, $patharr[0]]);
                    break;
            }
        } else {
            $upath = 'dzz:uid_' . $uid . ':' . $opath;
            if (!$fid = DB::result_first("select p.fid from %t p left join %t f on p.fid = f.fid where p.path = %s and f.deldateline < 1  ", [$this->_table, 'folder', $upath])) {
                $fid = DB::result_first("select  p.fid from %t p left join %t f on p.fid = f.fid where p.path regexp %s and f.deldateline < 1", [$this->_table, 'folder', '^dzz:.+:' . $path . '$']);
            }
        }
        return $fid;
    }
    ...
    public function update_path_by_fid($fid, $name) {
        ...
        $cachkey1 = [];
        $cachkey2 = [];
        foreach (DB::fetch_all("select fid from %t where path regexp %s", [$this->_table, '^' . $regpath . '.*']) as $v) {
            $cachkey1[] = 'patharr_' . $v['fid'];
            $cachkey2[] = 'path_' . $v['fid'];
        }
        $sql = "update %t set path = replace(path,%s,%s) where path regexp %s";
        if (DB::query($sql, [$this->_table, $path, $newpath, '^' . $regpath . '.*'])) {
    ...
    public function update_pathdata_by_fid($fid, $ofid, $noself = false) {
        if ($paths = $this->fetch_pathby_pfid($fid, true)) {
            ...
            $cachkey1 = [];
            $cachkey2 = [];
            foreach (DB::fetch_all("select fid from %t where path regexp %s", [$this->_table, '^' . $likepath]) as $v) {
                $cachkey1[] = 'patharr_' . $v['fid'];
                $cachkey2[] = 'path_' . $v['fid'];
            }
            if (DB::query($sql, [$this->_table, $path, $opath, $pathkey, $opathkey, '^' . $likepath])) {
                $this->clear_cache($cachkey1);
                $this->clear_cache($cachkey2);
                return true;
            } else {
    ...
    public function get_child_fids($fid) {
        ...
        $pathkey = $path['pathkey'];
        $fids = [];
        foreach (DB::fetch_all("select fid from %t where pathkey regexp %s", [$this->_table, '^' . $pathkey . '.*']) as $v) {
            $fids[] = $v['fid'];
        }
        return $fids;
    }
    ...
    public function parse_path_get_rootdirinfo($path) {
        ...
        $rootfid = DB::result_first("select fid from %t where path = %s order by fid DESC", [$this->_table, $rootpath]);
        if (!$rootfid) {
            return false;
        }
        $path = str_replace($rootpath, '', $path);
        return ['path' => $path, 'pfid' => $rootfid];
    }
}