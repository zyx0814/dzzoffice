...
class table_folder extends dzz_table {
    ...
    public function set_newperm_by_pfid($pfid, $perm, $force = false) {
        $pfid = intval($pfid);
        foreach (DB::fetch_all("select fid,perm from %t where pfid = %d", [$this->_table, $pfid]) as $v) {
            if (!$force && $v['perm'] > 0) {
                continue;
            } else {
                parent::update($v['fid'], ['perm' => 0, 'perm_inherit' => $perm]);
                $indexdata = ['fid' => $v['fid']];
                Hook::listen('changepermafter_updateindex', $indexdata);
    ...
    public function update_by_pfids($pfids, $setarr) {
        ...
        $fids = [];
        foreach (DB::fetch_all("select fid from %t where pfid in(%n)", [$this->_table, $pfids]) as $v) {
            $fids[] = $v['fid'];
        }
        foreach ($fids as $v) {
    ...
    public function update_perm_inherit_by_fid($fids) {
        ...
        foreach ($fids as $value) {
            ...
            parent::update($value, ['perm_inherit' => $perm_inherit]);
        }
    }
    ...
    public function insert($data, $appid = 0, $replace = false, $silent = false) {
        ...
        if (!$data['fname']) {
            return ['error' => lang('folder_name_can_not_empty')];
        }
        if ($path['fid'] = parent::insert($data, 1)) {
            $perm_inherit = perm_check::getPerm1($path['fid']);
            parent::update($path['fid'], ['perm_inherit' => $perm_inherit]);
            if ($data['pfid']) {
                ...
                if (!$pdata) {
                    return ['error' => lang('failure_newfolder')];
                }
    ...
    public function get_folder_pathinfo_by_fid($fid, $folderarr = [], $i = 0) {
        if (!$folderinfo = parent::fetch($fid)) return;

        array_unshift($folderarr, ['fname' => $folderinfo['fname'], 'fid' => $folderinfo['fid'], 'gid' => $folderinfo['gid'], 'uid' => $folderinfo['uid']]);
    ...
    public function check_home_by_uid($uid = '') {
        ...
        return DB::fetch_first("select f.*,p.path from %t f left join %t p on f.fid = p.fid where f.uid = %d and f.flag = %s", [$this->_table, 'resources_path', $uid, 'home']);
    ...
    public function fetch_home_by_uid($uid = '') {
        ...
        if ($return = DB::fetch_first("select f.*,p.path from %t f left join %t p on f.fid = p.fid where f.uid = %d and f.flag = %s", [$this->_table, 'resources_path', $uid, 'home'])) {
            return $return;
        } else {
            $root = [
                'pfid' => 0,
    ...
    public function fetch_topby_fname($fname) {
        $fid = DB::result_first("select fid from %t where fname = %s and pfid = 0", [$this->_table, $fname]);
    ...
    public function fetch_by_fid($fid, $gid = '') {
        global $_G;
        $fid = intval($fid);
        if (!$data = self::fetch($fid)) ['error' => lang('file_not_exist')];
        //获取目录的附加信息
    ...
    public function getFolderName($name, $pfid, $fid) {
        ...
        if (DB::result_first("select COUNT(*) from %t where fname=%s and  pfid=%d and fid != %d and isdelete<1", ['folder', $name, $pfid, $fid])) {
    ...
    //查询路径
    public function fetch_path_by_fid($fid, $fids = []) {
     ...
     */
    public function delete_by_fid($fid, $force = false) { //清空目录
        ...
        foreach (DB::fetch_all("select rid,oid,isdelete from %t where pfid = %d", ['resources', $fid, $fid]) as $v) {
            if ($v['isdelete'] > 0) {
                $isdelrids[] = $v['rid'];
            } else {
                $nodelrids[] = $v['rid'];
        ...
        if (!$force) {//如果非强制彻底删除，只删除删除状态项
            ...
            $delrids = $rids;
            //获取当前目录中在回收站中的数据
            foreach (DB::fetch_all("select rid from %t where pfid = %d", ['resources_recyle', $fid]) as $v) {
                $delrids[] = $v['rid'];
            }
        }
        //执行删除
    ...
    public function fetch_all_default_by_uid($uid) {
        return DB::fetch_all("SELECT * FROM %t WHERE `default`!= '' and uid=%d  ", [$this->_table, $uid], 'fid');
    }

    public function fetch_typefid_by_uid($uid) {
        $data = [];
        foreach (DB::fetch_all("SELECT * FROM %t WHERE `flag`!= 'folder' and  uid='{$uid}' and gid<1  ", [$this->_table], 'fid') as $value) {
    ...
    public function fetch_all_by_pfid($pfid, $count) {
        ...
        if ($count) return DB::result_first("SELECT COUNT(*) FROM %t WHERE $wheresql", [$this->_table, $pfid]);
        else return DB::fetch_all("SELECT * FROM %t WHERE $wheresql", [$this->_table, $pfid], 'fid');
    }

    public function fetch_folderinfo_by_gid($gid) {//查询群组目录及文件基本信息
        $gid = intval($gid);
        if ($info = DB::fetch_first("select f.*,p.path from %t f left join %t p on f.fid = p.fid  where gid = %d and flag = %s", [$this->_table, 'resources_path', $gid, 'organization'])) {
            return $info;
        }
        return false;
    }
    ...
    public function fetch_fid_by_flag($flag) {
        $uid = getglobal('uid');
        return DB::result_first("select fid from %t where uid = %d and  flag = %s", [$this->_table, $uid, $flag]);
    }

    public function fetch_fid_by_flags($flags) {
        if (!is_array($flags)) $flags = (array)$flags;
        $fids = [];
        foreach (DB::fetch_all("select fid from %t where flag in(%n)", [$this->_table, $flags]) as $v) {
            $fids[] = $v['fid'];
        }
        return $fids;
    }
    ...
    public function fetch_perm_by_fid($fid) {
        $perms = DB::fetch_first("select perm,perm_inherit from %t where fid = %d", [$this->_table, $fid]);
        if ($perms['perm']) {
            return $perms['perm'];
        } else {
    ...
    public function fetch_fsperm_by_fid($fid) {
        $data = [];
        if($perm = DB::fetch_first("select fsperm from %t where fid = %d", [$this->_table, $fid])) {
            $data = $perm;
        } else {
            return false;
        }
    ...
    public function fetch_folder_by_pfid($pfid, $field = []) {//查询群组目录及文件夹基本信息
        global $_G;
    ...
    public function fetch_fid_by_pfid($pfid) {
        ...
        return DB::fetch_all("select fid from %t $wheresql", [$this->_table, $pfid]);
    }
    ...
    //获取目录的所有上级目录
    public function fetch_all_parent_by_fid($fid, $ret = []) {
