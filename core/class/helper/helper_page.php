...
class helper_page {
	...
	public static function multi($num, $perpage, $curpage, $mpurl,$classname='', $maxpages = 0, $page = 5, $autogoto = FALSE, $simple = FALSE, $jsfunc = FALSE) {
		...
		if($num > $perpage) {
			...
			($curpage > 1 && !$simple ? '<li><a href="'.(self::mpurl($mpurl, $pagevar, $curpage - 1)).($ajaxtarget  && $autogoto ? '#' : $a_name).'" class="prev"'.$ajaxtarget.'>'.$lang['prev'].'</a></li>' : '');
			for($i = $from; $i <= $to; $i++) {
			...
			($curpage < $pages && !$simple ? '<li><a href="'.(self::mpurl($mpurl, $pagevar, $curpage + 1)).($ajaxtarget  && $autogoto ? '#' : $a_name).'" class="nxt"'.$ajaxtarget.'>'.$lang['next'].'</a></li>' : '').
