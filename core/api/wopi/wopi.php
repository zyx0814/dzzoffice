...
class Wopi
{
	 ...
	 * @param string $lock 文件锁
	 * @return array
	 */
	static function CheckFileInfo($path, $lock)
	{
		$meta = IO::getMeta($path);
		if (!$meta) helper_output::json(['error' => "path error"]);
		if ($meta['error']) helper_output::json(['error' => $meta['error']]);
		...
		helper_output::json($FileInfoDto);
	}
	...
	static function PutFile($path, $lock = '')
	{
		...
		if (intval($code) == 409) {
			...
			$return = IO::setFileContent($path, $content, true);
			if ($return['error']) {
