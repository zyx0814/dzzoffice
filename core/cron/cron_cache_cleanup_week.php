...
//清空临时缓存区
$time = 60 * 60 * 24; //1天 1天没有修改的将被删除；
removedirectory($_G['setting']['attachdir'] . 'cache/', true, $time);

//清理上传未成功的文件
$like = '%dzz_upload_%';
$like1 = '%FTP_upload_%';
foreach (DB::fetch_all("select * from %t where (cachekey like %s or cachekey like %s) and dateline<%d", ['cache', $like, $like1, TIMESTAMP - 24 * 60 * 60]) as $value) {
