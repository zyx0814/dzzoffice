...
### 问题修复
1. 修复云存储文件通过 `getStream` 接口预览时出现的**空白/加载失败**问题，优化传输逻辑。
...
### 注意事项
1. **离线升级**：从V2.01及以上版本的用户升级到V2.3.6需要执行升级程序。本次更新包含**关键问题修复**，建议所有用户升级。
2. **PHP版本**：仅支持php7.0+的PHP版本使用。
3. **清理缓存**：无论是在线升级还是离线升级都需要在升级后前往系统工具中清理系统缓存。

## DzzOffice 2.3.5 主要更新内容
